---
title: "Your library"
description: "Your owned items, what an access grant means, and how archived items behave."
---
...
[/dashboard/library](https://bkstr.tmrwgroup.ai/dashboard/library) lists items across three tabs:
...
**Free items never appear here.** The Library lists items you hold an access grant for. A free item is installable by anyone and is never granted to a specific account — so it is never owned, and never shows in the Library. Install free items directly; see [Installing](/docs/subscriber/installing).
...
The key is shown as its short prefix only — `bks_` plus eight characters. That prefix identifies the key; it is not the full secret. Copy your full key once, when you create it, from [/dashboard/api-keys](https://bkstr.tmrwgroup.ai/dashboard/api-keys) — bkstr stores only a hash of the key and cannot show it to you again. Installing and the CLI are covered in [Installing](/docs/subscriber/installing).
...
For the model behind grants, versions, and the books-versus-skills split, see [Concepts](/docs/reference/concepts).
