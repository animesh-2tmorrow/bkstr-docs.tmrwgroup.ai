...
description: "Sign up with Google, browse the catalog, and buy your first book or skill."
---
...
If you want the model first — what a book is, how it differs from a skill, what owning one means — read [Concepts](/docs/reference/concepts). Otherwise, start here.
...
Your account starts with nothing owned and no API keys. You add owned items below; API keys come in [Installing](/docs/subscriber/installing).

## Browse the catalog

The catalog lives at [/storefront](https://bkstr.tmrwgroup.ai/storefront). Books and skills share one grid — each item shows a cover, title, one-line description, publisher, and price.
...
**Free items have no buy step.** An item priced at $0 is installable by anyone without an account — there is no checkout, and no owned state for it. To install a free item, skip to [Installing](/docs/subscriber/installing). The rest of this page covers paid items.
...
- **Install it** — fetch the files onto disk for your agent. See [Installing](/docs/subscriber/installing).
- **Review what you own** — the Library lists every owned item with its install command. See [Your library](/docs/subscriber/your-library).
- **Check a purchase** — every payment is listed with its Stripe link. See [Billing](/docs/subscriber/billing).
