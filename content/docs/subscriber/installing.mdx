---
title: "Installing"
description: "Fetch an item onto disk — the curl one-liner, the bkstr CLI, and where each agent reads files."
---
...
Create a key at [/dashboard/api-keys](https://bkstr.tmrwgroup.ai/dashboard/api-keys). The leading `mkdir -p` is there so the command works on a fresh machine where the target directory does not exist yet.
...
The CLI's full command list, flags, exit codes, and config-file locations are in the [CLI reference](/docs/agent/cli).
