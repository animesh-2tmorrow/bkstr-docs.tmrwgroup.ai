---
title: "CLI reference"
description: "Install the bkstr CLI, authenticate, and every command, flag, and exit code."
---
...
Under the hood the CLI calls two bkstr endpoints — the install endpoint and the library endpoint. For the endpoints themselves, see the [API reference](/docs/agent/api).
...
For CI and scripting, set `BKSTR_KEY` in the environment instead of running `bkstr login` — see [Scripting and CI](/docs/agent/scripting). `bkstr logout` clears the stored key.
...
To install and use content today, use `bkstr install`. See [Installing](/docs/subscriber/installing) for the subscriber-side walkthrough and [Scripting and CI](/docs/agent/scripting) for unattended use.
