---
title: "MCP server"
description: "Connect Claude Code, Cursor, Codex CLI, or any MCP-compatible client to bkstr through the hosted MCP server."
---
...
The MCP server is a thin wrapper over the same HTTP endpoints documented under [API reference](/docs/agent/api) and [Q&A endpoint](/docs/agent/qa-endpoint); auth, grants, and rate limits are all enforced by the bkstr web app. If you prefer raw HTTP calls or a CLI, those paths still work — MCP is one more shape, not a replacement.
...
Create a key at [/dashboard/api-keys](https://bkstr.tmrwgroup.ai/dashboard/api-keys). Inside Claude Code the seven tools appear under the `bkstr` server in `/mcp`; refer to them by their bare names (`search_catalog`, `my_library`, …) — Claude Code handles the namespacing.
...
Visit [/dashboard/api-keys](https://bkstr.tmrwgroup.ai/dashboard/api-keys), click **Create new key**, give it a label, and copy the `bks_…` value the dialog shows. The key is shown once — paste it into your shell's `BKSTR_KEY` environment variable, or hold onto it for the next step.
...
Anonymous tools ignore the header. Authenticated tools without a key return `UNAUTHENTICATED`; with a key but no grant on the requested item, `NOT_OWNED`. Keys live at [/dashboard/api-keys](https://bkstr.tmrwgroup.ai/dashboard/api-keys); revoke a leaked one there and it stops authenticating immediately.
...
> **`NOT_OWNED` when loading or asking.** Your key is valid; you do not have an active grant on that item. Buy it on the storefront — `search_catalog` returns a `storefront_url` for every item, or browse [/storefront](https://bkstr.tmrwgroup.ai/storefront). After Stripe's webhook fires (usually seconds), the next `my_library` call lists the item.
