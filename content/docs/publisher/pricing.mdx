---
title: "Pricing"
description: "Set and change a book's price, and how bkstr keeps Stripe in sync."
---

Every book and skill on bkstr is sold for a one-time price in US dollars. This page covers setting and changing a book's price from the [Pricing](https://bkstr.tmrwgroup.ai/dashboard/pricing) page.

## The Pricing page

[/dashboard/pricing](https://bkstr.tmrwgroup.ai/dashboard/pricing) lists the books you have published, each with its current price and a control to change it. As a publisher you see your own books; an admin sees every book.
...
bkstr handles the payment processor for you. For a book that passes the [submission scan](/docs/publisher/becoming-a-publisher), bkstr creates the matching Stripe product and price when the book is published; for a book that was flagged for review, that happens when an admin approves it. When you change a price, bkstr creates a fresh Stripe price and points the book at it — Stripe prices are immutable, so a change is always a new price record, not an edit.
