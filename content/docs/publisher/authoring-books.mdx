...
description: "Create a book by pasting markdown or uploading a multi-chapter .zip."
---

A book is a bundle of markdown your readers' agents read as context. As a publisher, you create one from [/dashboard/books/new](https://bkstr.tmrwgroup.ai/dashboard/books/new). This page covers both ways to do that — pasting markdown and uploading a multi-chapter `.zip`.
...
- **Price** — US dollars, minimum $0.50. See [Pricing](/docs/publisher/pricing).
...
When you submit, your book is run through the automated submission scan. A book that passes is written — its first version, its price, your publisher grant, the Stripe product — and is live on its storefront page immediately. A submission the scan flags or rejects does not list; see [Becoming a publisher](/docs/publisher/becoming-a-publisher) for the three outcomes.
...
- **Skill bundles are rejected.** A zip whose root holds a `SKILL.md` is refused by the book path — to publish a skill, see [Authoring skills](/docs/publisher/authoring-skills).

A book that passes the submission scan is live on its storefront page right away.
...
A book that passes the [submission scan](/docs/publisher/becoming-a-publisher) is live the moment the form succeeds — it appears in the catalog's Browse tab for every signed-in subscriber. The scan catches secrets and malformed submissions, not content quality; there is no preview step.
...
- **Only the price is editable later.** Once a book is published, the title, slug, description, domain, and content cannot be changed from the dashboard — only the price, on the [Pricing](/docs/publisher/pricing) page. Decide the title and slug, and proofread the content, before you submit.

To re-price, archive, or check the catalog ledger, see [Pricing](/docs/publisher/pricing) and [Catalog management](/docs/publisher/catalog-management).
