...
description: "Archive and unarchive a book, and where to find the catalog ledger."
---
...
You archive a book from its row on the [Pricing](https://bkstr.tmrwgroup.ai/dashboard/pricing) page. Because archiving changes what buyers can find, the confirmation asks you to type the book's slug — a deliberate guard against an accidental archive.
...
Archiving is reversible — unarchiving a book returns it to the catalog with its price and content intact. Unarchiving re-runs the automated [submission scan](/docs/publisher/becoming-a-publisher), so a book whose content would no longer pass cannot be quietly returned to the storefront. (A book held or rejected by that scan's review queue cannot be archived at all — archiving is for live books.)
...
The catalog ledger is admin-only. As a publisher, the [Pricing](https://bkstr.tmrwgroup.ai/dashboard/pricing) page is your view of your own books.
