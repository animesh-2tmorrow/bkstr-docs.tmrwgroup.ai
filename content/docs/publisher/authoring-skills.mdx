...
description: "Package a skill as a .zip with a SKILL.md and supporting files."
---

A skill is a `.zip` bundle your readers' agents install and run — a `SKILL.md` plus the supporting files that make it work. You author a skill from the same form as a book, [/dashboard/books/new](https://bkstr.tmrwgroup.ai/dashboard/books/new), with the **Kind** toggle set to **Skill**.
...
A skill that passes the submission scan is live on its storefront page immediately, listed in the catalog alongside books — see [Becoming a publisher](/docs/publisher/becoming-a-publisher) for the scan and its three outcomes.
